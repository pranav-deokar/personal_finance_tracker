...
def add_expenses(user_name, income, month):
    ...
    def submit_expenses(user_name, income, month):  
        # TAKE values from entry fields and convert them to integers
...
def generate_report(user_name, income, expenses, total_savings, month, filename):
    ...
    if os.path.isfile(filename):
        ...
        for col, header in enumerate(headers, start=1):
            worksheet.cell(row=1, column=col, value=header)#UPDATE EACH TERM OF LIST AS COLUMN
    ...
    # Show message box with file save information(POPUP WINDOW)
...
def generate_graphs(report_window, expenses):
    ...
    if filtered_expenses:
        plt.bar([expense[1] for expense in filtered_expenses], labels=filtered_labels, autopct='%1.1f%%', startangle=140)
